...
import CreateReservationRules from './CreateReservationValidationRules.json';
...
export class CreateReservationController {
    ...
    public async handle(request: Request, response: Response) {
        try {
            const hasInvalidBody = ValidateBodyRequest(CreateReservationRules, request.body);
            if (hasInvalidBody) return ApiResponse.badRequest(response, 'Some fields are invalid', hasInvalidBody);

            const useCaseResponse = await this.createReservationUseCase.run(request.body);
